...
load(cellListPath)
recordingType = 'Excitation'; %********MANUALLY CHANGE***************
...
if isempty(needToAnalyze)
    disp('Could not find any cells that need preprocessing based on the current criteria')
    return
end
...
for i = 1:numel(indx)
    ...
    for e = 1:numel(epochNumbers)
        ...
        for j = 1:usableSecondsPerEpoch
            ...
            fragment = fragment - mean(fragment); %normalize to itself
    ...
    save(saveFname, 'allResponses', '-ascii')  % change the name of txt file
