function [resultsStruct, resultsStruct_batch] = Calibration_eyetracking(app)

%load stimulus, pupil, and daq data
...
for i = 1:2:numel(lightLevels_all)*2    
    ...
    radii_Left = []; %keeps track of pupil radius
...
stdDistanceByLightLevel = zeros(1, numel(lightLevels_unique));
meanPupilSizeByLightLevel = zeros(1, numel(lightLevels_unique));
stdPupilSizeByLightLevel = zeros(1, numel(lightLevels_unique));
for i = 1:numel(lightLevels_unique)
    ...
    if numel(distances_valid) == 0
        ...
        stdDistanceByLightLevel(i) = NaN;
        stdPupilSizeByLightLevel(i) = NaN;
        continue
    end
    
    meanDistanceByLightLevel(i) = mean(distances_valid);
    meanPupilSizeByLightLevel(i) = mean(pupilSizes_valid);
    stdDistanceByLightLevel(i) = std(distances_valid);
    stdPupilSizeByLightLevel(i) = std(pupilSizes_valid);
end
...
if numel(sideLED_frames) < numel(topLED_frames)
    topLED_frames = topLED_frames(1:numel(sideLED_frames));
end

topCRs_x = [];
topCRs_y = [];
for i = 1:numel(topLED_frames)
    trackingData = pupilData(topLED_frames(i), :);
...
sideCRs_y = [];
for i = 1:numel(topLED_frames)
    trackingData = pupilData(sideLED_frames(i), :);
    [pPosition, pRadius, CR] = findPupilPosition(trackingData, 0.5);
    if pPosition == -1 %bad frame
        continue
    end
...
resultsStruct.CalibrationDate = recordingDate;
resultsStruct.pupilRadii_means = meanPupilSizeByLightLevel;
resultsStruct.pupilRadii_std = stdPupilSizeByLightLevel;
resultsStruct.Rp_means = meanDistanceByLightLevel;
resultsStruct.Rp_std = stdDistanceByLightLevel;
resultsStruct.linearModel.CorrCoeff = R;
resultsStruct.linearModel.CorrCoeff_pval = p_value;
resultsStruct.linearModel.Slope = Slope;
resultsStruct.linearModel.Intercept = Intercept;
resultsStruct.linearModel.mdl = linearRpMdl;
resultsStruct.sideLEDDistance_x = sideDistance_x;
resultsStruct.sideLEDDistance_y = sideDistance_y;
...
resultsStruct.protocolName = app.UITable.Data{selection, 1}; %saves the name of the protocol alongside the calibration
resultsStruct.FrameHeight_recording = frameHeight;
resultsStruct.FrameWidth_recording = frameWidth;

resultsStruct_batch = resultsStruct; %for this stimulus the structure that is saved to the batch list is the same as the one saved for the individual cell
end